...
float T0_mut_sd; //T0 mutation standard deviation
float sigma_mut_sd; //sigma mutation standard deviation
float T0_ini_sd; //T0 initial standard deviation
float sigma_ini_sd; //sigma initial standard deviation
...
vector <int> sigma_m_count; //mutations in T0
vector <int> T0_m_count; //mutations in sigma
...
float temp_drate(float T){
     // float d_rate=D0*exp(-(Eb/(k*T)));
      float d_rate=D0*exp(-Ed/(k*T))+exp(-0.3*(T-273));
      return d_rate;
}
float s_T(float T,float A,float T0,float sigma,float r0){
	float temp_scale=r0*A*exp(-pow((T-T0),2)/(2*pow(sigma,2)));
	return temp_scale;
}
float temp_brate(int patch_no,float T,float A,float T0,float sigma,float r0){
      float temp_scale=r0*A*exp(-pow((T-T0),2)/(2*pow(sigma,2)));
...
void initialise(){
	...
	death_max.clear();  //patch-wise max death rate vector
        sigma_m_count.clear(); //mutations in T0
	T0_m_count.clear(); //mutations in sigma
	d_count.clear(); //mutations in dp


	//Calculate f0 and beta0 and D0
	f0=b/(A*exp(-pow((Tref-T0),2)/(2*pow(sigma,2))));
        beta0=Beta/(A*exp(-pow((Tref-T0),2)/(2*pow(sigma,2))));
	D0=d*exp((Ed)/(k*Tref));
	...
	for (int x=0;x<world_size;++x){
		...
		sum_beta.push_back(0);
                T0_m_count.push_back(0);
                sigma_m_count.push_back(0);
                d_count.push_back(0);
		...
		//Add inidividuals to patch
		    ...
		    newind.dp=disp+gauss(disp_ini_sd);
                    newind.T0=T0+gauss(T0_ini_sd);
                    newind.sigma=abs(sigma+gauss(sigma_ini_sd));
		    newind.r0=abs(r0+gauss(r0_ini_sd));
		    newind.patch_no=x;
		    sum_beta.at(x)=sum_beta.at(x)+beta0*s_T(patch_temp.at(x),A,newind.T0,newind.sigma,newind.r0);
                    
		    //Add inidividual to world
                    all_ind.push_back(newind);

		    //Track patch-wise max birth rate
		    if (birth_max.at(x)<temp_brate(x,patch_temp.at(x),A,newind.T0,newind.sigma,newind.r0)){
			 birth_max.at(x)=temp_brate(x,patch_temp.at(x),A,newind.T0,newind.sigma,newind.r0);
		    ...
		    if (death_max.at(x)<temp_drate(patch_temp.at(x))){
                         death_max.at(x)=temp_drate(patch_temp.at(x));
                    }
...
void birth(int x){
	...
	//Inherited properties
        ...
        //Check maximum/minimum patch front
	...
	if (ran()<mut_rate){
           baby.T0=all_ind[x].T0 +gauss(T0_mut_sd);
	   T0_m_count[baby.patch_no]=T0_m_count[baby.patch_no]+1;

        }
        else{
           baby.T0=all_ind[x].T0;
        }

        if (ran()<mut_rate){
  	   baby.sigma=abs(all_ind[x].sigma+gauss(sigma_mut_sd));
	   sigma_m_count[baby.patch_no]=sigma_m_count[baby.patch_no]+1;
        }
        else{
	   baby.sigma=all_ind[x].sigma;	
        }

	if (ran()<mut_rate){
           baby.r0=abs(all_ind[x].r0+gauss(r0_mut_sd));
        }
        else{
           baby.r0=all_ind[x].r0;
        }
        
	baby.mut_eff=f0*s_T(patch_temp.at(baby.patch_no),A,baby.T0,baby.sigma,baby.r0)- f0*s_T(patch_temp.at(all_ind[x].patch_no),A,all_ind[x].T0,all_ind[x].sigma,all_ind[x].r0);
	
	float beta_sT=beta0*s_T(patch_temp.at(baby.patch_no),A,baby.T0,baby.sigma,baby.r0);
        sum_beta.at(baby.patch_no)=sum_beta.at(baby.patch_no)+beta_sT;
	//Add individ0ual
	all_ind.push_back(baby);
        
	//Check max birth rate
	if (temp_brate(baby.patch_no,patch_temp.at(baby.patch_no),A,baby.T0,baby.sigma,baby.r0)>birth_max[baby.patch_no]){
            birth_max[baby.patch_no]=temp_brate(baby.patch_no,patch_temp.at(baby.patch_no),A,baby.T0,baby.sigma,baby.r0);
	    
        }
        //Check max death rate
	if (temp_drate(patch_temp.at(baby.patch_no))>death_max[baby.patch_no]){
            death_max[baby.patch_no]=temp_drate(patch_temp.at(baby.patch_no));
        }
...
void death(int x){
	...
	float T0_d=all_ind[x].T0;
	float r0_d=all_ind[x].r0;
	float sigma_d=all_ind[x].sigma;
	...
	float beta_sT=beta0*s_T(T,A,T0_d,sigma_d,r0_d);
        sum_beta.at(patch_no)=sum_beta.at(patch_no)-beta_sT;
	//Check patch birth max
	if (birth_max.at(patch_no)==temp_brate(patch_no,T,A,T0_d,sigma_d,r0_d)){
		birth_max.at(patch_no)=0;
		for (int i=0;i<Ntot;++i){
			if (all_ind.at(i).patch_no==patch_no){
				if (birth_max.at(patch_no)<temp_brate(patch_no,T,A,all_ind[i].T0,all_ind[i].sigma,all_ind[i].r0)){
					birth_max.at(patch_no)=temp_brate(patch_no,T,A,all_ind[i].T0,all_ind[i].sigma,all_ind[i].r0);
				}
			}
		}
	}
	...
	if (death_max.at(patch_no)==temp_drate(T)){
                death_max.at(patch_no)=0;
                for (int i=0;i<Ntot;++i){
                        if (all_ind.at(i).patch_no==patch_no){
                                if (death_max.at(patch_no)<temp_drate(T)){
                                        death_max.at(patch_no)=temp_drate(T);
                                }
                        }
                }
        }
...
void dispersal(int x){
	...
	if (ran()>disp_mort){
          ...
          float beta_sT_home=beta0*s_T(patch_temp[home_patch],A,all_ind.at(x).T0,all_ind.at(x).sigma,all_ind[x].r0);
          sum_beta.at(home_patch)=sum_beta.at(home_patch)-beta_sT_home;
          float beta_sT_target=beta0*s_T(patch_temp[target_patch],A,all_ind.at(x).T0,all_ind.at(x).sigma,all_ind[x].r0);
	  ...
	  if (birth_max.at(target_patch)<temp_brate(target_patch,patch_temp[target_patch],A,all_ind.at(x).T0,all_ind.at(x).sigma,all_ind[x].r0)){
		 birth_max.at(target_patch)=temp_brate(target_patch,patch_temp[target_patch],A,all_ind.at(x).T0,all_ind.at(x).sigma,all_ind[x].r0);
	  }

	  //Track patch-wise birth rate at home
          if (birth_max.at(home_patch)==temp_brate(home_patch,patch_temp[home_patch],A,all_ind.at(x).T0,all_ind.at(x).sigma,all_ind[x].r0)){
               birth_max.at(home_patch)=0;
                for (int i=0;i<Ntot;++i){
                        if (all_ind.at(i).patch_no==home_patch){
                                if (birth_max.at(home_patch)<temp_brate(home_patch,patch_temp[home_patch],A,all_ind.at(i).T0,all_ind.at(i).sigma,all_ind[i].r0)){
                                        birth_max.at(home_patch)=temp_brate(home_patch,patch_temp[home_patch],A,all_ind.at(i).T0,all_ind.at(i).sigma,all_ind[i].r0);
                                }
                        }
                }
          }
          ...
          if (death_max.at(target_patch)<temp_drate(patch_temp[target_patch])){
			death_max.at(target_patch)=temp_drate(patch_temp[target_patch]);
          }

          //Track patch-wise birth rate at home
          if (death_max.at(home_patch)==temp_drate(patch_temp[home_patch])){
               death_max.at(home_patch)=0;
                for (int i=0;i<Ntot;++i){
                        if (all_ind.at(i).patch_no==home_patch){
                                if (death_max.at(home_patch)<temp_drate(patch_temp[home_patch])){
                                        death_max.at(home_patch)=temp_drate(patch_temp[home_patch]);
                                }
                        }
                }
          }
    ...
    for (int i=0;i<Ntot;++i){
      if (all_ind.at(i).patch_no==x){
         ...
         if (avg==0){
           ...
           avg=(all_ind.at(i).r0+avg)/2;
         }
         vec.push_back(all_ind.at(i).r0);
...
vector <float> average_sigma(int x){
   float avg=0;
   vector <float> vec;
   vector <float> stats;
   if (patch_density.at(x)==0){
    ...
    for (int i=0;i<Ntot;++i){
      if (all_ind.at(i).patch_no==x){
         int pop_dens=patch_density.at(x);
         if (avg==0){
                 avg=all_ind.at(i).sigma;
         }
         else{
           avg=(all_ind.at(i).sigma+avg)/2;
         }
         vec.push_back(all_ind.at(i).sigma);
    ...
    for (int i=0;i<Ntot;++i){
      if (all_ind.at(i).patch_no==x){
         ...
         if (avg==0){
           ...
           avg=(all_ind.at(i).T0+avg)/2;
         }
         vec.push_back(all_ind.at(i).T0);
...
void bd_event(){
  ...
  if (event_ran < (c_birth_max)/(c_birth_max+c_death_max+c_disp_max)) {
	...
	int N=patch_density[patch_no];
	float T=patch_temp[patch_no];
	float c_birth=temp_brate(patch_no,T,A,all_ind.at(x).T0,all_ind.at(x).sigma,all_ind[x].r0);
 ...
 //Potential Death or Dispersal eventexp(-Ea/(K*T))
	...
	if (event_ran>(c_birth_max+c_death_max)/(c_birth_max+c_death_max+c_disp_max)){
	      int x=floor(ran()*Ntot);
	      int patch_no=all_ind[x].patch_no;
...
int main(int argc,char* argv[]){
  ...
  //specify_rng(RS);   //Generate random number
  ...
  for (int run_no=0;run_no<tot_runs;++run_no) {
	  ...
	  check_max_rate();
          ...
          // headers
	  outputindiv <<"run"<< " " <<"patch"<<" "<<"time"<< " " <<"cons"<<" "<<"dispersal_events"<<" "<<"T0_mut_count"<<" "<<"sigma_mut_count"<<" "<<"average_dp"<<" "<<"q25_dp"<<" "<<"q75_dp"<<" "<<"average_T0"<<" "<<"q25_T0"<<" "<<"q75_T0"<<" "<<"average_sigma"<<" "<<"q25_sigma"<<" "<<"q75_sigma"<<" "<<"average_r0"<<" "<<"q25_r0"<<" "<<"q75_r0" <<" "<<"average_mut_eff"<<" "<<"q25_mut_eff"<<" "<<"q75_mut_eff"<<" "<<"sum_beta"<<endl;
          //average dispersal rate
	  vector <float> avg_T0;
	  vector <float> avg_sigma;
	  vector <float> avg_r0;
	  vector <float> avg_dp;
	  ...
	  //dispersal event counter
        ...
        //            disp_on=false;
	      ...
	      if (int(t)>=iter){
	        for (int x=0;x<world_size;++x){
		  avg_T0=average_T0(x);
		  avg_sigma=average_sigma(x);
		  avg_r0=average_r0(x);
                  avg_dp=average_dp(x);
		  avg_mut_eff=average_mut_eff(x);
		  outputindiv <<j<< " " <<x<<" "<<t<< " " <<patch_density[x]<<" "<<d_count[x]<<" "<<T0_m_count[x]<<" "<<sigma_m_count[x]<<" "<<avg_dp[0]<<" "<<avg_dp[1]<<" "<<avg_dp[2]<<" "<<avg_T0[0]<<" "<<avg_T0[1]<<" "<<avg_T0[2]<<" "<<avg_sigma[0]<<" "<<avg_sigma[1]<<" "<<avg_sigma[2]<<" "<<avg_r0[0]<<" "<<avg_r0[1]<<" "<<avg_r0[2]<<" "<<avg_mut_eff[0]<<" "<<avg_mut_eff[1]<<" "<<avg_mut_eff[2]<<" "<<sum_beta[x]<<endl; //Update output file
	          b_count=0;
		  T0_m_count[x]=0;
                  sigma_m_count[x]=0;
                  d_count[x]=0;
		...
		if (iter%1==0 and int(t)>=2000){
	        //list_ind.push_back(all_ind);
		     ...
		     // headers
                     file <<"individual"<< " " <<"T0"<<" "<<"sigma"<< " " <<"disp_rate"<<" "<<"r0"<<" "<<"patch_no"<<" "<<"mut_eff"<<endl;
                     for (int x=0;x<all_ind.size();++x){
                       if (all_ind[x].patch_no==max_pf or all_ind[x].patch_no==max_pf-1 or all_ind[x].patch_no==max_pf-2 or all_ind[x].patch_no==min_pf or  all_ind[x].patch_no==min_pf+1 or all_ind[x].patch_no==min_pf+2 or all_ind[x].patch_no==int(world_size/2)) {
                         ...
                         file <<x<< " " <<all_ind[x].T0<< " " <<all_ind[x].sigma<<" "<<all_ind[x].dp<<" "<<all_ind[x].r0<<" "<<all_ind[x].patch_no<<" "<<all_ind[x].mut_eff<<endl; //Update output file
	        ...
	        iter=iter+time_step;	
		      ...
		      avg_dp=average_dp(x);
		      avg_mut_eff=average_mut_eff(x);
		      outputindiv <<j<< " " <<x<<" "<<t<< " " <<patch_density[x]<<" "<<d_count[x]<<" "<<T0_m_count[x]<<" "<<sigma_m_count[x]<<" "<<avg_dp[0]<<" "<<avg_dp[1]<<" "<<avg_dp[2]<<" "<<avg_T0[0]<<" "<<avg_T0[1]<<" "<<avg_T0[2]<<" "<<avg_sigma[0]<<" "<<avg_sigma[1]<<" "<<avg_sigma[2]<<" "<<avg_r0[0]<<" "<<avg_r0[1]<<" "<<avg_r0[2]<<" "<<avg_mut_eff[0]<<" "<<avg_mut_eff[1]<<" "<<avg_mut_eff[2]<<" "<<sum_beta[x]<<endl;
                    }
